...
- ⏰ **定时自动化**：GitHub Actions 在北京时间 07:37–09:17 使用离峰滚动窗口；首个成功快照后，同一上海自然日的后续运行自动跳过。
...
- 触发：北京时间 07:37–09:17 每 10 分钟滚动尝试（UTC cron 分布在 23/00/01 时）+ `workflow_dispatch` / `repository_dispatch`。每次先检查上海自然日和 12 小时新鲜度，首个成功快照后其余运行自动跳过。
